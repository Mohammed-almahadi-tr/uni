...
## 0.0 Corrections made while building (v2.1.0 - v2.9.0)
...
| 31 | §2.1's Student Admission row is **corrected and widened**: the legacy student is spread across **four** tables, not two, and the four Arabic names are keyed **twice** by two clerks on two screens | `StdForm` ([FrmStudForm2.vb:379](Nile%20College%20E-University%20System/Oasis%20-%20E-University/Registration%20System/Forms/FrmStudForm2.vb#L379)) and `StdData` ([FrmDataEntery.vb:55](Nile%20College%20E-University%20System/Oasis%20-%20E-University/Registration%20System/Forms/FrmDataEntery.vb#L55)) receive the same four names from independent forms with nothing reconciling them, and the search dialog reads only `StdData`. `StudentsProfilees` and `StudentsProfilesIndecent` hold the accepted and rejected copies. A name corrected on one screen stays wrong on every other. The older, richer admission form `FrmForm.vb` has its **entire save commented out** ([lines 336-400](Nile%20College%20E-University%20System/Oasis%20-%20E-University/Registration%20System/Forms/FrmForm.vb#L336-L400)) |
| 32 | REQ-ST-01 gains: the displayed name is **derived from the four parts and checked against them by the database** | The legacy build wrote `x = x1 + " " + x2 + " " + x3 + " " + x4` inline in four separate screens over nullable columns. In VB a DBNull in that expression throws, so one student missing a fourth name broke the whole profile list rather than displaying a short name. Two representations of one fact is the shape of every reconciliation problem in this audit; a name is not an exception |
| 33 | REQ-ST-02 restated: the legacy medical form **validated four name fields it then discarded**, and stored *not tested* and *screened negative* identically | The save refuses until all four Arabic names are entered ([FrmMedical.vb:73-83](Nile%20College%20E-University%20System/Oasis%20-%20E-University/Registration%20System/Forms/FrmMedical.vb#L73-L83)) and the insert that follows names six columns, none of them a name. `AddWithValue("@Aids", Me.CombAids.Text)` on an unset combo box passes the empty string, and the validation that would have caught it is commented out along with the rule making HIV screening conditional on the nursing programme. A screening result is therefore a **three-valued** field, not a string |
| 34 | REQ-ST-02 gains: a medical record is **append-only and superseded, never edited**, with one current record per student | `MedicalExamination` had no key of any kind and the form inserted a fresh row on every save, so a student examined twice had two rows and nothing said which was current — the profile screen read them with `While reader.Read` assigning each to the same control and kept whichever came back last. A finding recorded on a date is what a decision taken that week was based on, which is the same reason a posted voucher is corrected by reversal rather than by edit |
| 35 | REQ-ST-05 gains three rules the build showed to be necessary: documents **supersede rather than replace**, the **verifier is never the uploader**, and a renewable document **must carry an expiry** | `frmStudentProfiles` is the third screen in this codebase to `DELETE` before inserting ([lines 233-236](Nile%20College%20E-University%20System/Oasis%20-%20E-University/Registration%20System/Forms/frmStudentProfiles.vb#L233-L236)), with `' Trans.Commit()` commented out on the line below, so a failure between the two statements leaves the student with no profile at all. A document uploaded and verified by one person has been checked by nobody. A residence permit with no expiry never reaches the expiry report, which is how an institution discovers in June that its foreign students are out of status |
| 36 | REQ-LIF-01's state diagram is **corrected**: there is no `Rejected` student status, and `Applicant --> Rejected` is removed. A rejected applicant is an **application** with a REJECT decision (Module 17) and never becomes a student record | Making rejection a student standing is precisely the legacy mistake. `StudentsProfilesIndecent` is a table of people the institution decided were **not** its students, keyed and reported as though they were — and because `frmStudentProfiles` deletes only from the table it is about to write ([lines 232-289](Nile%20College%20E-University%20System/Oasis%20-%20E-University/Registration%20System/Forms/frmStudentProfiles.vb#L232-L289)), a student whose verdict changes ends up in both tables at once, with `Update StdForm Set CH=1` run either way so the admission form cannot tell them apart |
| 37 | REQ-FEE-03's "raises either a refund payable or a retained credit balance" is **corrected**: they are the same liability, and the election is recorded rather than posted differently | An unmatched credit already sits in the Student Credit Balance control account (correction 5), which is money the institution owes. There is no second account for a refund payable to raise it into; electing a refund records the student's choice and the disbursement is a Module 16 payment voucher against a student rather than a vendor. Posting a separate payable would report one debt twice |
| 38 | REQ-SPN-01's "the sponsor's AR sub-ledger account" is **corrected**: sponsors share one Sponsor AR **control account** and are distinguished by sub-ledger identity, not by an account each | An account per sponsor is the legacy mistake — identity as a name — in a new place, and it grows the chart without bound: a hundred embassies would be a hundred level-5 accounts nobody reconciles. The control-account pattern already carries per-party detail for students (REQ-CSH-01) and vendors, and `reconcileSponsorSubledger` compares the sum of sponsor shares against the one control balance exactly as A3 does for students |
| 39 | REQ-LP-05's CMS is **narrowed**: semester start and end dates and the registration deadline are **read from `academic_terms`**, not authored in the website. The CMS carries only what the system has no counterpart for — examinations, holidays, ceremonies | Taken literally the requirement produces a second copy of a date the registration engine already enforces (`registration_closes_on`, against which `assert_registration_term_open` refuses a late registration). The two disagree the first time a registrar extends a deadline without telling whoever edits the site, and the student believes the website. `chk_calendar_event_not_derived` refuses to store the three derived kinds at all, so the drift is not available to be introduced |
| 40 | REQ-LP-03's "published tuition schedules" is **corrected**: the public figure is resolved from the approved, effective-dated fee schedule the registration engine bills from, and is **absent rather than stale** when none is in force | A separately published fee is the same defect as `CollegeFees.TuitionFees` beside `Transactions.TuitionFees` — two records of one price, which is the subtraction the legacy discount reports were built on ([viewDiscount](Nile%20College%20System%20-%20Ribat%20Univ/discount%20view.txt)). B1 made a schedule immutable and versioned precisely so there is one answer per cohort per day; the website is another reader of it, not another author |
| 41 | REQ-LP-01 gains: branding colours are stored as **HSL channels with range checks**, and typography is chosen from a **shipped allow-list** rather than typed | An out-of-range channel does not fail loudly — it emits an invalid CSS declaration, the custom property falls back to its initial value, and the university's site renders in a colour nobody chose, on a page the vendor never loads. A free-text font name is both an injection surface and a way to select a face with no Arabic coverage, which breaks joining mid-word on every Arabic page |
...
| **Student Admission / Profile** | `FrmStudForm2.vb`, `FrmDataEntery.vb`, `frmStudentProfiles.vb`, `FrmForm.vb`, `StdForm`, `StdData`, `StudentsProfilees`, `StudentsProfilesIndecent` | **Four tables, no key between any of them.** `FrmStudForm2` writes the four Arabic and four English names to `StdForm`; `FrmDataEntery` writes the same four Arabic names again to `StdData` on a separate screen, and the student search dialog reads only that second copy. `StudentsProfilees` holds accepted students with the name **concatenated into one column**; `StudentsProfilesIndecent` holds rejected ones. `FrmForm.vb`, the older and richer admission screen, has its **entire save commented out**. The admission date is not recorded but manufactured — `dat = dat1.AddMinutes(10)`, ten minutes after the previous student in that faculty. The profile save runs `DELETE` then `INSERT` with the transaction commented out, and its list query is filtered on a **hard-coded** faculty name | One student, one profile. Discrete four-part names in both languages, checked against the displayed name by database trigger; demographics, dual-calendar dates, guardian and secondary-school record in named columns; the profile seeded from the admission application rather than re-keyed | **Rebuilt** |
| **Medical Fitness Check** | `FrmMedical.vb`, `MedicalExamination` | Records exactly six fields: `UniversityID`, `DateofMedicalExamination`, `Hepatitis`, `Aids`, `BooldType`, `Employee`. The form **validates four name fields it then discards** — the insert names none of them. An unset screening combo box is stored as the **empty string**, so *never tested* and *screened negative* are indistinguishable. `Employee` is the data-entry clerk, so no record names who examined anyone. There is **no key on the table**, so a student examined twice has two rows and nothing says which is current, and there is **no vaccination status, no chronic conditions, and no fitness verdict** | Append-only medical records, one current per student, superseded rather than edited. The six legacy fields typed — screening results three-valued, blood group an enum — plus vaccinations, chronic conditions, allergies, officer notes, a Fit/Conditional/Unfit verdict with a mandatory reason, a validity date, and the clinician named separately from the clerk | **Rebuilt** |
...
- **REQ-ST-02: Medical Fitness & Examination Records**
  ...
  - A screening result is **three-valued**: Not Tested, Negative, Positive. The legacy form stored an unselected combo box as the empty string, making a student never screened indistinguishable from one screened negative.
  - The **clinician who examined the student is recorded by name**, separately from the staff member who keyed the record. The legacy `Employee` column held only the latter, so no record names who examined anyone.
  - A verdict other than Fit **must carry its reason**, enforced by CHECK. A conditional clearance nobody can read the condition of, and an unfit verdict with no stated basis, are not appealable.
  - Records are **append-only**, one current per student. A re-examination supersedes the previous record; neither editing nor deleting an examination is possible. A finding recorded on a date is what a decision taken that week was based on.
  - A lapsed clearance is reported as **Lapsed**, a state distinct from Unfit. A certificate that has run out is an examination to repeat, not a finding of unfitness.
- **REQ-ST-03: Advanced Student Search & Filter**
  - Debounced search across University ID, National ID, all four name parts in both languages, Program, Faculty, Batch, Admission Category, Nationality, and Status.
  - Backed by a **GIN trigram index** on the normalised key. An unanchored substring match is otherwise a sequential scan, and REQ-NFR-01's sub-100ms figure over 100k students is unreachable rather than merely missed. *(The legacy query was `Where StdFirName like N'<typed>%'` — a prefix of the first name only, so a family name matched nothing.)*
  - Every search term shall be required, so a second word narrows the result rather than widening it.
...
- **REQ-ST-05: Student Document Checklist**
  ...
  - Requirements are **additive**: declaring one shall not remove others. Removing a requirement is a separate, explicit act. *(The legacy codebase deleted-then-reinserted in three separate screens; this is the shape of code where that happens.)*
  - Uploading a replacement **supersedes** the previous document rather than replacing it, with exactly one live document per type per student enforced by partial unique index. A replaced scan is the evidence of what was checked previously and shall not be destroyed.
  - **The verifier shall never be the uploader**, enforced by database constraint rather than only by role. A document uploaded and marked verified by one person has been verified by nobody.
  - A document of a type marked renewable **shall carry an expiry date**, enforced by trigger. A renewable document with no expiry can never appear on the expiry report.
  - A document whose expiry has passed is **not** a satisfied requirement, whatever its verification state at the time it was checked.
  - Every document records its content digest at upload, so a later audit can prove the file served is the file that was verified. The bytes behind a document shall not be swapped.
...
12. `students` (id, tenant_id, student_no, national_id, full_name_ar, full_name_en, name_ar_1..4 **†**, name_en_1..4 **†**, search_key **†**, status, admitted_on, programme_id **†**, batch_id **†**, admission_category_id **†**, nationality_id **†**) — the four dimensions may be filled in but never cleared, by trigger; the displayed name is checked against the parts by trigger
12a. `student_profiles` **†** (id, tenant_id, student_id, gender, date_of_birth, birth_calendar **†**, place_of_birth, religion, marital_status, passport_no, passport_expiry, email, phone, whatsapp, address, guardian_*, emergency_*, school_name, certificate_type_id, certificate_seat_no, certificate_year, certificate_score) — split from `students` so the cashier's hot path does not carry forty profile columns
...
15. `document_types` **†** (id, tenant_id, code, name_ar, name_en, requires_expiry, sort_order), `programme_document_requirements` **†** (id, tenant_id, programme_id, document_type_id, is_mandatory)
15a. `student_documents` **†** (id, tenant_id, student_id, document_type_id, file_name, content_type, byte_size, storage_key, sha256, issued_on, expires_on, state, verified_by_id, verified_at, rejection_reason, superseded_at, uploaded_by_id) — one live document per type per student by partial unique index; `verified_by_id <> uploaded_by_id` by CHECK
15b. `medical_records` **†** (id, tenant_id, student_id, exam_date, blood_group, hepatitis_b, hiv, vaccinations, chronic_conditions, allergies, officer_notes, verdict, verdict_note, valid_until, medical_officer, recorded_by_id, superseded_at) — append-only; one current record per student by partial unique index
