 ...
 *
 * The shipped emerald and gold from `globals.css`, so an unconfigured tenant
 * looks like the product rather than like a broken page. The fallback is
 * deliberate: a missing row must degrade to something legible, not to the
 * browser's idea of an unset custom property.
 *
 * The accent's lightness is above `inkFor`'s threshold on purpose — gold is a
 * pale surface, and derived white ink on it is the unreadable button the
...
export const DEFAULT_BRANDING: BrandingTokens = {
  ...
  mottoAr: null,
  mottoEn: null,
  logoUrl: '/oau-logo.png',
  logoDarkUrl: '/oau-logo.png',
  faviconUrl: '/oau-logo.png',
  primary: { h: 163, s: 78, l: 22 },
  secondary: { h: 162, s: 84, l: 16 },
  accent: { h: 44, s: 58, l: 62 },
  headingFont: 'Cairo',
  bodyFont: 'Cairo',
};
